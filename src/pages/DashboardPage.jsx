...
export function DashboardPage() {
  ...
  const averageAttendance = data.users.length
    ? Math.round(
        data.users.reduce((s, u) => s + u.attendanceRate, 0) /
          data.users.length,
      )
    : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <Badge tone="blue">
          <BellRing className="h-3 w-3" />0 deadline minggu ini
        </Badge>
      </motion.div>
      ...
      <div className="grid items-start gap-4 xl:grid-cols-2">
        {/* Line chart */}
        <ChartCard
          title="Task Selesai per Bulan"
          subtitle="Tren penyelesaian task"
          delay={0}
        >
          <LineChart
            data={data.analytics}
          ...
          >
            ...
            <Line
              ...
              stroke={PALETTE[0]}
              strokeWidth={3.5}
              dot={{ r: 4.5, fill: "#fff", stroke: PALETTE[0], strokeWidth: 2.5 }}
              activeDot={{
                r: 6,
                fill: PALETTE[0],
                stroke: "#fff",
        ...
        {/* Bar chart layanan */}
        <ChartCard
          title="Kategori Layanan Pajak"
          subtitle="Jumlah layanan per kategori"
          delay={0.1}
        >
          <BarChart
            data={taxCategoryChart}
            margin={{ top: 8, right: 12, left: -12, bottom: 4 }}
          >
            <CartesianGrid {...gridStyle} vertical={false} />
            <XAxis
              dataKey="category"
              tick={{ ...axisTick, angle: -45, textAnchor: "end", height: 40 }}
              axisLine={false}
              tickLine={false}
            />
            <YAxis
        ...
        {/* Area chart point */}
        <ChartCard
          title="Tren Point Staff"
          subtitle="Akumulasi point bulanan"
          delay={0.3}
        >
          <AreaChart
            data={data.analytics}
          ...
          >
            ...
            <Area
              ...
              strokeWidth={3.5}
              fill="url(#gradPoints)"
              dot={{ r: 4.5, fill: "#fff", stroke: PALETTE[2], strokeWidth: 2.5 }}
              activeDot={{
                r: 6,
                fill: PALETTE[2],
                stroke: "#fff",
        ...
        {/* Radar */}
        <ChartCard
          title="Radar Operasional"
          subtitle="Performa lintas dimensi"
        ...
        >
          <RadarChart
            ...
            data={[
              ...
              { item: "Task", score: data.tasks.length },
          ...
          >
            ...
            <Radar
              ...
              fill="url(#radarGrad)"
              strokeWidth={3}
              dot={{ r: 4.5, fill: "#fff", stroke: PALETTE[4], strokeWidth: 2 }}
              {...lineAnimationProps}
            />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: "var(--tw-colors-slate-500)", opacity: 0.05, rx: 4 }} />
          </RadarChart>
