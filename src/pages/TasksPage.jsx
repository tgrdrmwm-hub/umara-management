  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Task Board</h1>
        ...
        <Card className="p-5">
          ...
          <form className="grid gap-3 sm:grid-cols-2" onSubmit={(e) => void submit(e)}>
            <div className="space-y-1 sm:col-span-2">
              <label className="text-xs font-medium text-slate-600 dark:text-slate-400">Judul Task *</label>
              <Input placeholder="Deskripsi singkat task" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
            </div>
            ...
            <div className="space-y-1">
              ...
              <Input placeholder="Pisahkan dengan koma" value={form.pic} onChange={(e) => setForm({ ...form, pic: e.target.value })} />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-slate-600 dark:text-slate-400">Point</label>
            ...
            <div className="flex gap-2 sm:col-span-2">
              <Button type="submit">{editing ? "Simpan Perubahan" : "Tambah Task"}</Button>
              ...
              <div className="space-y-2">
                ...
                {colTasks.map((task) => (
                  <Card key={task.id} className="p-3 shadow-none hover:shadow-sm transition-shadow">
                    ...
                    {task.client && (
                      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{task.client}</p>
                    )}
                    {task.deadline && (
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      {task.pic && (
                        <span className="truncate text-xs text-slate-500 dark:text-slate-400 max-w-[100px]">{task.pic}</span>
                      )}
                      <Badge tone={col.key === "done" ? "green" : "slate"} className="ml-auto shrink-0">
                        {task.points} pts
                      </Badge>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-1.5 border-t border-slate-100 pt-2.5 dark:border-white/8">
                      ...
                      {col.key !== "done" && (
                        <Button
                          size="sm"
                          onClick={() =>
                            void updateTask(task.id, { ...task, status: "done" }, task.status)
                              .then(() => refresh("Task selesai, point ditambahkan"))
                          }
                        >
                          Selesai
                        </Button>
                      ...
                      <Button
                        ...
                        onClick={() => void deleteTask(task.id).then(() => refresh("Task dihapus"))}
                      >
                        Hapus
                      </Button>
                    </div>
