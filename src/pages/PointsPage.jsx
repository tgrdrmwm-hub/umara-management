...
export function PointsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Point & Leaderboard</h1>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {[
          ...
          { label: "Staff Terunggul", value: topStaff?.name ?? "—", sub: topStaff ? `${topStaff.points} poin` : undefined },
          ...
          <motion.div key={stat.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07, duration: 0.4 }}>
            <Card className="p-4">
              ...
              <p className="mt-2 text-2xl font-semibold text-slate-900 dark:text-slate-100 truncate">{stat.value}</p>
      ...
      >
        <Card className="p-5">
          <div className="mb-5">
            <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Grafik Point Staff</h2>
            <p className="mt-0.5 text-xs text-slate-400">Peringkat dari tertinggi ke terendah</p>
          </div>
          {ranked.length === 0 ? (
            <div className="flex h-48 items-center justify-center text-sm text-slate-400">Belum ada data</div>
          ) : (
            <div className="h-56 sm:h-64 lg:h-72 w-full min-w-0 min-h-0">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={ranked} margin={{ top: 8, right: 12, left: -12, bottom: 4 }}>
                  <CartesianGrid {...gridStyle} vertical={false} />
                  <XAxis dataKey="name" tick={{ ...axisTick, angle: -45, textAnchor: "end", height: 40 }} axisLine={false} tickLine={false} />
                  <YAxis tick={axisTick} axisLine={false} tickLine={false} allowDecimals={false} />
                  <Tooltip
                    {...tooltipStyle}
                    formatter={(v, _, props) => [v.toLocaleString() + " pt", props.payload.name]}
                  />
                  <Bar dataKey="points" name="Point" radius={[6, 6, 0, 0]} maxBarSize={56} {...animationProps}>
                    {ranked.map((_, i) => (
                      <Cell
                        key={i}
                        fill={i < 3 ? podiumColors[i] : PALETTE[i % PALETTE.length]}
            ...
            <table className="w-full min-w-[520px] text-xs sm:min-w-[560px] sm:text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-white/8">
                  <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400">Rank</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/8">
                {ranked.map((user, i) => {
                  ...
                  return (
                    <tr key={user.id} className={i === 0 ? "bg-amber-50/60 dark:bg-amber-500/5" : ""}>
                      ...
                      <td className="px-4 py-3 capitalize text-slate-600 dark:text-slate-400">{user.role.replace("_", " ")}</td>
                      <td className="px-4 py-3"><Badge tone={badgeConfig[badge].tone}>{badge}</Badge></td>
                      <td className="px-4 py-3">
                        <span className="text-base font-semibold" style={{ color: i < 3 ? podiumColors[i] : undefined }}>
